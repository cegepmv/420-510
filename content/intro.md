...
{{< figure
    src="/420-510/images/soutien-canada.png"
...
{{< figure
    src="/420-510/images/agent-soutien-tech.png"
    alt="Agent de soutien technique — portrait de la profession"
    caption="[Fiche métier — Agent de soutien technique](https://www.macarrieretechno.com/fr/carrières/professions/agent-de-soutien-technique)"
...
> Les **titres de postes** varient, mais suivent souvent les **niveaux N1 → N2 → N3** (de l’accueil à l’expertise).
...
<!-- opérateurs N0-1 : service cleint et travail manuel (remplacer ancres imprimantes, gérer le papier, projecteur fonctionne ps, fillage)
technicien nrml (L1/L2) : probs d'accès, 
technicien classe principale (L2/L3) : trv ps le prob ou sln (complexité du prob) parfois question de rôle (catégorie de prob à gérer : exemple telephone IP - un technicien qui a été formé par la compagnie et donc c'est la personne ressource)
*nommé aisni pour tout ce qui gouv, sys scolaire, santé etc.
dans classe principale il y a des postes comme admin reso, developpeurs 
analyste fonctionnel (N3/N4) : gérer prob complexe, directement en contact avec prestataires (venir faire un projet à l'école, comme installer des caméras-donc compagnie externe) lui s'occupe de projet (prob touche une oersonne s'arrete souvent à N3) s'il soccupe d'un prob utilisateur c'est question d'accès : exemple supprimer contenu du onedrive à part annlyste (accès speciaux pcq lois entre en jeux)

niv hirarchique + je monte + je suis payé + de responsabilité et accès et prob
CP on volet de sécurité  -->
<!-- 1. ervice client et interventions matérielles simples : remplacement de cartouches d’encre/toner, gestion du papier, vérification/raccordement du câblage, dépannage de base des projecteurs/écrans, préparation de postes et petits périphériques; escalade vers L1 pour incidents logiciels ou cas complexes.
2.   
3.   
-->

<!-- AV & Facilities Support = soutien audiovisuel et services généraux / logistique des locaux. -->
| Terme (FR) | Anglais courant | Rôle principal | Niveau typique |
|---|---|---|---|
| **Opérateur·rice TI** | IT Support Operator / Service Desk Operator / AV & Facilities Support | Service client et interventions matérielles simples : vérification/raccordement du câblage, dépannage de base des projecteurs/écrans, préparation de postes et petits périphériques. **Escalade vers N1** pour incidents logiciels ou cas complexes. | N0-N1 | 
| **Agent·e de soutien aux utilisateurs** | Service desk / Help desk analyst | **Première ligne (N1)** : accueille, qualifie, crée le ticket, résout les cas simples, escalade si besoin. | N1 |
| **Technicien·ne en soutien informatique** *(poste de travail/terrain)* | Desktop/Workplace Support, Field Tech | Interventions **pratiques** sur les postes/perifs : préparation du matériel, installations, dépannages sur site. | N1–N2 |
| **Technicien·ne classe principale** *(terme courant dans le public : gouv., scolaire, santé)* | Senior/Principal Technician (SME – Subject Matter Expert) / Systems Administraton | **Première ligne (N1)** : accueille, qualifie, crée le ticket, résout les cas simples, escalade si besoin. | N2-N3 |
| **Analyste fonctionnel·le** | Functional Analyst / Business Systems Analyst / Application Support Analyst | Spécialiste d’une **application** : repro des bugs, paramètres, correctifs, lien avec l’équipe produit. Gestion des dossiers complexes et transverses : nalyse des besoins, droits d’accès & conformité, coordination avec prestataires (ex. déploiement de caméras), pilotage de petits projets. En contact direct avec les prestataires | N3-N4 |
| **Support technique** *(au sens large)* | Technical Support | Ensemble des **interventions techniques** au-delà de l’accueil (diagnostic poussé, corrections). | N2–N3 |
| **Admin. systèmes / réseaux / cloud** | Sysadmin / Network / Cloud Engineer | Gère **serveurs, réseau, sécurité** ; intervient en **escalade** et pour la stabilité de la plateforme. | N3 |
...
### En bref
- **Niveaux (N1, N2, N3, N4)** = **qui** traite le ticket (compétence / profondeur technique).
- **Sévérité (S1…S5)** = **à quel point c’est grave** pour l’organisation (impact + urgence).
- On peut donc avoir un **ticket S1 traité par N3**, ou un **ticket S4 traité par N1** : ce sont **deux axes distincts**.
...
<!-- - mdp perdu pr se connecter N1 - S1
- imprimante pas d'ancre L0 - S4 -->
  


## Entrer en contact avec le support technique

Le support technique est généralement offert pour **rétablir rapidement l’usage** des outils numériques et **accompagner** les personnes utilisatrices lorsqu’un incident ou une demande dépasse leur autonomie. Le service technique centralise le **triage**, la **traçabilité** (ticket) et l’**escalade** vers les bonnes équipes afin d’assurer **continuité, sécurité et qualité**.

### Les principaux moyens de contact
<details> <summary>📞 <strong>Appel téléphonique</strong></summary>

À privilégier pour : urgences, blocages de production, incidents majeurs (S1–S2).

</details> <details> <summary>💬 <strong>Chat en direct</strong></summary>

Idéal pour : assistance rapide en temps réel, qualification initiale, premiers tests guidés.

</details> <details> <summary>🧾 <strong>Portail / Système de tickets</strong></summary>

Idéal pour : traçabilité, suivi, requêtes standardisées.

</details> <details> <summary>✉️ <strong>Courriel</strong></summary>

Idéal pour : décrire calmement un problème non urgent, avec besoin de détailler le contexte.

</details> <details> <summary>🖥️ <strong>Assistance à distance</strong></summary>

Idéal pour : prise de contrôle (TeamViewer/AnyDesk/Quick Assist) afin de reproduire et corriger.

</details> <details> <summary>🏢 <strong>Sur site / en magasin</strong></summary>

Idéal pour : matériel défectueux, diagnostics physiques.

</details> <details> <summary>👥 <strong>Forums & communauté</strong></summary>

Idéal pour : produits grand public, astuces d’éditeur/utilisateurs.

</details> <details> <summary>📚 <strong>FAQ / documentation</strong></summary>

Idéal pour : problèmes récurrents et auto-assistance rapide.

</details> <details> <summary>📣 <strong>Réseaux sociaux</strong></summary>

À privilégier pour : suivi public d’un incident côté éditeur, messages privés pour du support léger.

</details> <details> <summary>🎥 <strong>Tutoriels & webinaires</strong></summary>

À privilégier pour : procédures d’installation, configuration ou bonnes pratiques d’usage.

</details>

<!-- exos infos à fournir ? -->

### Infos à fournir (quel que soit le canal)

- **Description claire** du problème + **symptômes** / message d’erreur exact.

- **Étapes pour reproduire** (1–2–3), **depuis quand**, **contexte** (poste, réseau, lieu).

- **Impact** (bloquant ou non), **nombre d’utilisateurs** touchés, **priorité** souhaitée.

- **Environnement** : OS/version, appli/version, pare-feu/antivirus, matériel impliqué, application.

- **Captures d’écran** / court enregistrement, **solutions déjà essayées**.

- Vos **coordonnées** et **plage de disponibilité** pour un rappel/prise en main.
